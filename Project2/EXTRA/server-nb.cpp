...
int main(int argc, char *argv[]) {
  ...
  // Main loop
  fd_set mainset; // The 'ground truth' set for the non-blocking descriptors
  fd_set fdset; // Since select changes the fd_set it reads, this is a
                // temporary list for the descriptors
  int fdmax; // Maximum file descriptor number - to keep track of it

  FD_ZERO(&mainset); // Clear the sets
  FD_ZERO(&fdset);

  // Connections can be pending if many concurrent client requests
  listen(ssocket, MAX_PENDING);
  FD_SET(ssocket,&mainset);
  fdmax = ssocket;
  ...
  while(true) { // Loop forever...
    // Selecting
    fdset = mainset; // Copy main set
    if (select(fdmax+1,&fdset,NULL,NULL,NULL) < 0) {
      errorexit("Select had an error");
    }
    // Find which one is set
    for (int i = 0; i <= fdmax; ++i) {
      if (FD_ISSET(i,&fdset)) { // Found it
        // Case 1: We got a new connection
        if (i == ssocket) {
          // Wait for a message
          ...
          FD_SET(csocket,&mainset);
          fdmax = max(fdmax,csocket);
        }
        // Case 2: We can recieve from another message
        else {
          csocket = i;
  ...
  // And if all goes well we will never reach here :D
  close(ssocket);
}
